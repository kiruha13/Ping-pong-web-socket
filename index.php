...
<!-- Toastr используем для красивых уведомлений в браузере -->
<script type="text/javascript" src="https://cdnjs.cloudflare.com/ajax/libs/toastr.js/latest/js/toastr.min.js"></script>

<script>
    //Настраиваем уведомления для пользователей
    toastr.options = {
        "closeButton": false,
        "debug": false,
        "newestOnTop": false,
        "progressBar": false,
        "positionClass": "toast-bottom-left",//то, где будут наши уведомления располагаться
...
<script type="text/javascript">
    ...
    var turn = '', end_of_game = 1;

    $(document).ready(function () {
        var wsUri = "ws://127.0.0.1:65522/server.php";
        ...
        websocket.onmessage = function (ev) {
            ...
            var uname = msg.name;
            var go = msg.end_of_game;
            if (type == 'system') {
                if (turn == '') {
                    turn = uname;
                    toastr["info"]("You are : " + turn + " side . Hope enjoy")
                }
                if (go == 1) end_of_game = 0;
                else end_of_game = 1;
...
<script>
    ...
    var left_racket, right_racket, ball, side = 'left', direction_set = 0, right_score, left_score, sound_strike,
        sound_gameover, sound_shoes1, sound_shoes2;
    var top_line = 235, bottom_line = 450, right_line = 445, left_line = 40, center_line = 320, center = 240;//ограничиваем наше поле относительно картинки стола

    function get_random_direction() {
        var random = Math.floor((Math.random() * 2) + 1);//рандом выдает значения, которые округляем с помощью floor
        return random;
    }
    ...
    //описываем все компоненты нашей игры и их положение в самом начале
    function startGame() {
        ...
        if (end_of_game == 0) {
            if (direction_set == 0) {
                ...
                if (ball.x + 5 == left_line && Math.abs(left_racket.y - ball.y) < 25) {
                    if (side == 'left')
                        side = 'right'; else side = 'left';
                } else if
                (ball.x + 5 == left_line && Math.abs(left_racket.y - ball.y) >= 25) {
                    // right goal
                    right_score.text += 1;
                    change_side();
                    // alert('right goal');
